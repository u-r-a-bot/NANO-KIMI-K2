...
class Trainer:
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            ...
            for batch in iterable:
                ...
                logits, _ = self.model(input_ids, start_pos=0, use_cache=False)
    ...
    def plot_metrics(self):
        ...
        try:
            ...
            axes[0, 1].plot(self.metrics['steps'], self.metrics['train_ce_loss'], label='CE Loss', alpha=0.7, color='orange')
            ...
            axes[0, 2].plot(self.metrics['steps'], self.metrics['train_aux_loss'], label='Aux Loss', alpha=0.7, color='red')
            axes[0, 2].set_xlabel('Steps')
            axes[0, 2].set_ylabel('Aux Loss')
            axes[0, 2].set_title('Router Aux Loss (load balancing)')
            axes[0, 2].legend()
            axes[0, 2].grid(True)
            ...
            if len(self.metrics['train_loss']) > 50:
                ...
                axes[1, 1].plot(self.metrics['steps'][:len(smoothed_loss)], smoothed_loss)
                axes[1, 1].set_xlabel('Steps')
                axes[1, 1].set_ylabel('Smoothed Loss')
                axes[1, 1].set_title('Smoothed Training Loss')
                axes[1, 1].grid(True)

            axes[1, 2].plot(self.metrics['steps'], self.metrics['grad_norm'])
    ...
    def train(self):
        ...
        panel = Panel.fit(
            ...
            f"Aux Loss Coeff: {self.aux_loss_coeff}\n"
                ...
                while self.global_step < self.max_steps:
                    ...
                    if self.mixed_precision:
                        with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                            logits, aux_loss = self.model(input_ids, start_pos=0, use_cache=False)
                            ce_loss = F.cross_entropy(logits.reshape(-1, self.model.vocab_size), labels.reshape(-1))
                            loss = ce_loss + self.aux_loss_coeff * aux_loss
                    ...
                    accumulated_max_logits = max(accumulated_max_logits, logits.abs().max().item())
                    accumulated_loss += loss.item()
                    accumulated_ce_loss += ce_loss.item() / self.gradient_accumulation_steps
                    accumulated_aux_loss += aux_loss.item() / self.gradient_accumulation_steps
                    ...
                    if accumulation_counter >= self.gradient_accumulation_steps:
                        ...
                        if self.mixed_precision:
                            self.scaler.step(self.optimizer, max_logits=accumulated_max_logits)
                            self.scaler.update()
                        else:
                            self.optimizer.step(max_logits=accumulated_max_logits)
                        ...
                        self.metrics['train_ce_loss'].append(accumulated_ce_loss)
                        self.metrics['train_aux_loss'].append(accumulated_aux_loss)
                        ...
                        self.progress.update(
                            ...
                            advance=1,
                            description=(
                                f"[cyan]Step {self.global_step}/{self.max_steps} | "
...
def main():
    ...
    parser.add_argument('--val_from_train', action='store_true', default=False)
    parser.add_argument('--val_batches', type=int, default=8)
    parser.add_argument('--num_workers', type=int, default=2)
    parser.add_argument('--refresh_optimizer_state', action='store_true', default=False)
    ...
    if hasattr(dataset, '__len__'):
        ...
        console.print(f"[green]Streaming dataset loaded[/green]")
    ...
    if args.val_from_train and args.eval_interval > 0:
        ...
        del dl_iter
        console.print(f"[green]Cached {len(cached_val_samples)} validation batches[/green]")
    ...
    training_success = False  # initialized before try so finally block never crashes
    ...
    try:
        ...
        for obj in ['trainer', 'model', 'dataloader', 'dataset', 'tokenizer']:
            try:
                del locals()[obj]
            except:
                pass
    ...
    exit_code = 0 if (training_success and not exception_occurred) else 1
